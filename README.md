# MMDect - 多模态无人机集群人机交互系统

基于ROS/Gazebo的多模态指令识别与无人机集群控制系统

[![Version](https://img.shields.io/badge/version-6.3-blue.svg)](https://github.com/)
[![Python](https://img.shields.io/badge/python-3.8+-green.svg)](https://www.python.org/)
[![License](https://img.shields.io/badge/license-MIT-orange.svg)](LICENSE)

---

## 🚀 快速开始（5分钟）

```bash
# 1. 安装依赖
pip install -r requirements.txt

# 2. 下载模型（可选，首次运行会自动下载）
bash scripts/download_models.sh

# 3. 测试摄像头
python tools/test_camera.py

# 4. 测试手势识别
python src/detectors/gesture_detector.py

# 5. 测试高级可视化（带指尖轨迹）
python test_gesture_advanced.py

# 6. 启动完整系统
python src/main.py --enable-all --unified-viewer
```

---

## ✨ 系统特性

### 四种输入模态

| 模态 | 模型 | 大小 | 延迟 | 说明 |
|-----|------|------|------|------|
| 🎤 **语音** | Vosk | ~40MB | <100ms | 中文关键词识别 |
| ✋ **手势** | MediaPipe | ~2MB | ~33ms | 21点手部跟踪，5种手势 |
| 📷 **图像** | YOLOv8-nano | ~6MB | ~40ms | 符号标记+目标检测 |
| 🖱️ **鼠标** | RDP算法 | - | <10ms | 路径规划 |

### 支持的指令

- ✈️ **起飞** (Takeoff) - 语音"起飞" / 张开手掌(5指) / 符号T
- 🛬 **降落** (Land) - 语音"降落" / 握拳(0指) / 符号H
- ⏸️ **悬停** (Hover) - 语音"悬停" / 单指 / 符号L
- 🔍 **探索** (Explore) - 语音"探索" / V手势(2指) / 符号S
- 🎯 **编队** (Formation) - 语音"编队" / 三指 / 符号F
- 📍 **航点** (Waypoint) - 鼠标绘制路径
- 🚨 **紧急停止** (Emergency Stop) - 语音"紧急停止"

---

## 📖 完整文档

**➡️ [DOCUMENTATION.md](DOCUMENTATION.md) - 统一完整文档**

包含以下所有内容：
1. 项目概述
2. 快速开始
3. 安装指南
4. 使用手册
5. 系统架构
6. 检测器详解
7. 配置说明
8. 故障排除
9. 开发指南
10. 性能指标
11. 项目状态

---

## 🎯 核心亮点

### 统一架构
所有检测器继承`BaseDetector`基类，保证接口一致性和代码复用。

### 轻量化模型
总模型大小 < 50MB，可运行在边缘设备和笔记本电脑。

### 实时性能
端到端延迟 < 200ms，满足实时控制需求。

### 多模态融合
4种融合策略（最新/置信度/投票/优先级），时间窗口融合（0.5秒）。

### 即开即用
笔记本摄像头即可测试，无需额外硬件。

---

## 📁 项目结构

```
MMDect/
├── src/
│   ├── detectors/              # 4个检测器
│   │   ├── base_detector.py    # 统一基类 ⭐
│   │   ├── voice_detector.py   # 语音检测
│   │   ├── gesture_detector.py # 手势识别
│   │   ├── image_detector.py   # 图像检测
│   │   └── mouse_detector.py   # 鼠标路径
│   ├── core/                   # 核心模块
│   │   ├── command_manager.py  # 多模态融合 ⭐
│   │   └── camera_manager.py   # 相机共享
│   ├── visualization/
│   │   └── unified_viewer.py   # 统一查看器
│   └── main.py                 # 主程序 ⭐
├── models/                     # 预训练模型
├── config/
│   └── detector_config.yaml    # 配置文件 ⭐
├── tools/                      # 测试工具
├── test_gesture_advanced.py    # 高级手势测试
├── requirements.txt            # Python依赖
├── DOCUMENTATION.md            # 完整文档 📚
└── README.md                   # 本文件
```

---

## 🛠️ 快速命令

### 测试单个检测器
```bash
python tools/test_camera.py              # 测试摄像头
python src/detectors/gesture_detector.py # 手势识别
python test_gesture_advanced.py          # 高级可视化（带轨迹）
python src/detectors/voice_detector.py   # 语音识别
python src/detectors/image_detector.py   # 图像检测
python src/detectors/mouse_detector.py   # 鼠标路径
```

### 完整系统
```bash
# 启用所有检测器 + 统一查看器
python src/main.py --enable-all --unified-viewer

# 启用特定组合
python src/main.py --gesture --image --mouse

# 调试模式
python src/main.py --enable-all --log-level DEBUG
```

### 统一查看器按键
- `1` - 手势检测视图
- `2` - 图像检测视图
- `3` - 鼠标路径视图
- `4` - 四宫格分屏
- `0` - 概览视图
- `R` - 重置统计
- `Q` - 退出

---

## 📊 性能指标

| 指标 | 数值 | 状态 |
|-----|------|------|
| 端到端延迟 | < 150ms | ✅ |
| 语音识别准确率 | ~85% | ✅ |
| 手势识别准确率 | ~95% | ✅ |
| 图像检测准确率 | ~75% | ✅ |
| 总模型大小 | ~48MB | ✅ |
| CPU占用 | ~65% | ✅ |
| 内存占用 | ~950MB | ✅ |

*测试环境：Intel i5-8代, 8GB RAM, 笔记本前置摄像头*

---

## 🔧 故障排除

### 摄像头打不开
```bash
# 检测可用摄像头
python tools/test_camera.py --find

# 尝试不同索引
python src/detectors/gesture_detector.py --camera 0
python src/detectors/gesture_detector.py --camera 1
```

### 模型文件找不到
```bash
# 重新下载模型
bash scripts/download_models.sh

# 检查模型文件
ls -lh models/
```

### 手势识别不准确
- 保持简单背景
- 确保光线充足
- 距离摄像头 30-60cm
- 调整配置文件中的置信度阈值

**更多问题？** 查看 [DOCUMENTATION.md](DOCUMENTATION.md) 第8章：故障排除

---

## 🎓 学习路径

### 🏃 快速上手（30分钟）
1. 阅读本README（5分钟）
2. 运行快速开始命令（15分钟）
3. 查看 [DOCUMENTATION.md](DOCUMENTATION.md) 第4章（10分钟）

### 📚 深入理解（2小时）
1. 阅读 [DOCUMENTATION.md](DOCUMENTATION.md) 完整文档
2. 浏览 `src/detectors/` 源代码
3. 测试各个检测器

### 🔨 自定义开发（半天）
1. 完成"深入理解"路径
2. 查看 [DOCUMENTATION.md](DOCUMENTATION.md) 第9章：开发指南
3. 训练自定义模型

---

## 📈 项目状态

**当前版本**: v1.0 Phase 1 ✅

**已完成**：
- ✅ 4个检测器全部实现
- ✅ 多模态融合系统
- ✅ 相机共享机制
- ✅ 统一查看器
- ✅ 完整文档

**待实现** (Phase 2)：
- ⏸️ ROS/Gazebo集成
- ⏸️ MAVROS/PX4接口
- ⏸️ 无人机控制器
- ⏸️ Web可视化界面

---

## 🌟 参考项目

- [RACER (HKUST/SYSU-STAR)](https://github.com/SYSU-STAR/RACER)
- [EGO-Planner-Swarm (ZJU FAST-Lab)](https://github.com/ZJU-FAST-Lab/ego-planner-swarm)
- [Fast-Planner (HKUST)](https://github.com/HKUST-Aerial-Robotics/Fast-Planner)

---

## 📄 许可证

MIT License

---

## 📞 获取帮助

- 📖 完整文档：[DOCUMENTATION.md](DOCUMENTATION.md)
- 🐛 问题反馈：guowei_ni@bit,edu.cn

---

**快速链接**：
- [完整文档](DOCUMENTATION.md)
- [快速开始](#-快速开始5分钟)
- [故障排除](#-故障排除)
- [性能指标](#-性能指标)

**最后更新**: 2025-11-06
